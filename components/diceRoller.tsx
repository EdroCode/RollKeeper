...
import { BrushCleaning, Dices, OctagonX } from "lucide-react";
  ...
  return (
    <div className="p-4 rounded-2xl bg-linear-tp from-zinc-50 to-zinc-100 border border-zinc-200 shadow-sm w-md max-w-xl min-h-80 flex flex-col">
      <header className="flex justify-between items-center mb-3 shrink-0 w-full">
        <h1 className="flex gap-2 items-center text-[10px] font-semibold tracking-[0.2em] text-zinc-500">
          <Dices size={16} className="text-zinc-900" />
          DICE ROLLER
        </h1>
      ...
      <main className="grid grid-cols-2 gap-2 flex-1 min-h-0 w-full">
        <div className="flex flex-col gap-2 w-48">
            ...
            {dice.map((value) => (
              <button
                ...
                className="
                  ...
                  shadow-sm text-zinc-700 text-xs font-semibold
          ...
          <div className="flex gap-2">
            <div className="flex flex-col gap-1 w-20">
              ...
              <input
                ...
                className="p-1.5 text-xs rounded-md border border-zinc-200 bg-white/60 text-zinc-900"
              />
            </div>

            <div className="flex flex-col gap-1 flex-1">
              <p className="text-[10px] text-zinc-500">Dice</p>
              <input
                type="number"
                min={2}
                max={100}
          ...
          <div className="flex flex-col gap-1">
            <p className="text-[10px] text-zinc-500">Modifier</p>
            <input
              type="number"
              value={modifier}
          ...
          <button
            ...
            className="mt-1 px-3 py-2 rounded-xl bg-zinc-900 text-white text-xs hover:bg-zinc-800 active:scale-95 transition font-medium"
          >
        ...
        <div className="flex flex-col gap-3 min-h-0">
          <div className="flex-1 flex flex-col items-center justify-center rounded-xl bg-white/70 border border-zinc-200 shadow-inner p-4 overflow-hidden">
            <h2 className="text-6xl font-black text-zinc-900 leading-none">
              {result ?? "00"}
            </h2>

            {breakdown.length > 0 && (
              <p className="text-[10px] text-zinc-400 mt-2 wrap-break-words text-center">
                {breakdown.join(" + ")}
                {modifier !== 0
                  ? ` ${modifier >= 0 ? "+" : ""}${modifier}`
                  : ""}
          ...
          <div className="h-36 shrink-0 p-3 rounded-xl bg-white/40 border border-zinc-200 shadow-sm overflow-y-auto">
            ...
            <div className="space-y-1">
              {history.map((h, i) => (
                <div key={i} className="text-xs text-zinc-700 wrap-break-words">
                  {h}
                </div>
              ))}
            </div>
